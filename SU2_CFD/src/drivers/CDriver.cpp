        ...
        if (compressible) {
          ...
          switch (config->GetKind_Upwind_Flow()) {
            ...
            case UPWIND::HLLC:
              ...
              break;
            case UPWIND::HLLC_SLAU2:
              if (ideal_gas) {
                for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
                  numerics[iMGlevel][FLOW_SOL][conv_term] = new CUpwHLLC_PLUS_SLAU2_Flow(nDim, nVar_Flow, config);
                  numerics[iMGlevel][FLOW_SOL][conv_bound_term] = new CUpwHLLC_PLUS_SLAU2_Flow(nDim, nVar_Flow, config);
                }
              }
              else {
                for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
