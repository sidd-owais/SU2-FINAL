...
#include "../../../../include/numerics/flow/convection/hllc_slau2.hpp"
#include "../../../../../Common/include/toolboxes/geometry_toolbox.hpp"

CUpwHLLC_PLUS_SLAU2_Flow::CUpwHLLC_PLUS_SLAU2_Flow(unsigned short val_nDim, unsigned short val_nVar, const CConfig* config) : CNumerics(val_nDim, val_nVar, config) {
...
CUpwHLLC_PLUS_SLAU2_Flow::~CUpwHLLC_PLUS_SLAU2_Flow() {
...
CNumerics::ResidualType<> CUpwHLLC_PLUS_SLAU2_Flow::ComputeResidual(const CConfig* config) {
...
//cout<< MassFlux << " " << Pressure <<endl;
  ...
  //RoeSoundSpeed  =  (SoundSpeed_i * sqrt(Density_i) + SoundSpeed_j * sqrt(Density_j))/Rrho - ProjInterfaceVel;  
//cout<< RoeSoundSpeed - ((SoundSpeed_i * sqrt(Density_i) + SoundSpeed_j * sqrt(Density_j))/Rrho - ProjInterfaceVel )<<endl;
  ...
  if (sM > 0.0) {
    ...
    if (sL > 0.0) {
      ...
      for (iVar = 0; iVar < nVar; iVar++)
        dpStar_dU[iVar] = Density_j * (sR - ProjVelocity_j) * dSm_dU[iVar];
      ...
      for (iVar = 0; iVar < nVar; iVar++)
        dpStar_dU[iVar] = Density_i * (sL - ProjVelocity_i) * dSm_dU[iVar];
...
void CUpwHLLC_PLUS_SLAU2_Flow::ComputeMassAndPressureFluxes(const CConfig* config, su2double &mdot, su2double &pressure) {
